...
def reco_bdkpimumu(df, is_mc=False):
    ...
    reco_bdkpimumu_collection = JaggedCandidateArray.candidatesfromcounts(
        ...
        l1_eta             = df["BToKsMuMu_lep1eta_fullfit"].flatten(),
        l1_phi             = df["BToKsMuMu_lep1phi_fullfit"].flatten(),
        l1_pt              = df["BToKsMuMu_lep1pt_fullfit"].flatten(),
        l2_eta             = df["BToKsMuMu_lep2eta_fullfit"].flatten(),
        l2_phi             = df["BToKsMuMu_lep2phi_fullfit"].flatten(),
        l2_pt              = df["BToKsMuMu_lep2pt_fullfit"].flatten(),
...
def reco_bukmumu(df, is_mc=False):
    reco_bukmumu_collection = JaggedCandidateArray.candidatesfromcounts(
        ...
        l1_eta       = df["BToKMuMu_fit_l1_eta"].flatten(),
        l1_phi       = df["BToKMuMu_fit_l1_phi"].flatten(),
        l1_pt        = df["BToKMuMu_fit_l1_pt"].flatten(),
        l2_eta       = df["BToKMuMu_fit_l2_eta"].flatten(),
        l2_phi       = df["BToKMuMu_fit_l2_phi"].flatten(),
        l2_pt        = df["BToKMuMu_fit_l2_pt"].flatten(),
