...
def prefit_swap(tree, mass_range=BD_FIT_WINDOW, incut="1"):
	...
	signal_model = ROOT.RooExtendPdf("signal_model", "signal_model", signal_sumpdf, nsignal)
...
def prefit_main(tree, mass_range=BD_FIT_WINDOW, incut="1"):
	ws = ROOT.RooWorkspace('ws')
	...
	signal_pdf = make_signal_pdf_main(ws, mass_range)
	nsignal = ws.factory(f"nsignal[{ndata*0.5}, 0.0, {ndata*2.0}]")
...
def fit_mc(tree, mass_range=BD_FIT_WINDOW, incut="1", cut_name="inclusive"):
	...
	constraints = {}
	with open("Bd/prefitparams_MC_Bd_frozen.pkl", 'rb') as f:
		prefit_params = pickle.load(f)
	with open("Bd/prefiterrs_MC_Bd_frozen.pkl", 'rb') as f:
...
if __name__ == "__main__":
	...
	if args.prefits:
		...
		for side in ["recomatch", "recomatchswap"]:
			...
			for cut_name in cuts:
				...
				if side == "recomatch":
					ws, fit_result = prefit_main(chain, incut=cut_str)
				else:
					ws, fit_result = prefit_swap(chain, incut=cut_str)
				ws.Print()
				fit_result.Print()

				plot_fit(ws, tag="prefit_Bd_{}_{}".format(side, cut_name), text=fit_text[cut_name])

				ws_file = ROOT.TFile("Bd/prefitws_mc_Bd_{}_{}.root".format(side, cut_name), "RECREATE")
				ws.Write()
				fit_result.Write()
				ws_file.Close()
		...
		pprint(final_errors)
		if args.all:
			with open("Bd/prefitparams_MC_Bd.pkl", "wb") as f:
				pickle.dump(final_params, f)
			with open("Bd/prefiterrs_MC_Bd.pkl", "wb") as f:
	...
	if args.fits:
		...
		for cut_name in cuts:
			cut_str = cut_strings[cut_name]
			plot_mc(chain, cut=cut_str, tag="Bd_{}".format(cut_name))
	...
	if args.plots:
		for cut_name in cuts:
			...
			#ws_file.ls()
			ws = ws_file.Get("ws")
			plot_fit(ws, tag="Bd_{}".format(cut_name), text=fit_text[cut_name])
	...
	if args.tables:
		...
		pprint(final_errors)
		if args.all:
			with open("Bd/fitparams_MC_Bd.pkl", "wb") as f:
				pickle.dump(final_params, f)
			with open("Bd/fiterrs_MC_Bd.pkl", "wb") as f:
				pickle.dump(final_errors, f)
