...
class MCEfficencyProcessor(processor.ProcessorABC):
  def __init__(self):
    self._nominal_trigger = "HLT_Mu9_IP5|HLT_Mu9_IP6" # "HLT_Mu9_IP6"
  ...
  def process(self, df):
    ...
    nominal_trigmask = trigger_masks[self._nominal_trigger]
    ...
    selections["sv_prob"]  = (reco_bukmumu.sv_prob > 0.07)
    ...
    selections["trigger"] = nominal_trigmask * reco_bukmumu_mask_template
    ...
    for trigger in self._triggers:
      ...
      selections[f"recotrig_{trigger}"]   = selections["reco"] & (trigger_mask * reco_bukmumu_mask_template).astype(bool)
      selections[f"tag_{trigger}"]            = selections[f"recotrig_{trigger}"] & (reco_bukmumu.Muon1IsTrig | reco_bukmumu.Muon2IsTrig)
      selections[f"tagmatch_{trigger}"]       = selections[f"tag_{trigger}"] & (reco_bukmumu.genPartIdx >= 0)
      selections[f"tagunmatched_{trigger}"]   = selections[f"tag_{trigger}"] & (reco_bukmumu.genPartIdx < 0)
      
      selections[f"probe_{trigger}"]          = selections[f"recotrig_{trigger}"] & (reco_bukmumu.TagCount >= 1)
      selections[f"probematch_{trigger}"]     = selections[f"probe_{trigger}"] & (reco_bukmumu.genPartIdx >= 0)
      selections[f"probeunmatched_{trigger}"] = selections[f"probe_{trigger}"] & (reco_bukmumu.genPartIdx < 0)
    ...
    output["reco_cutflow"][dataset_name]["tag"] += selections[f"tag_{self._nominal_trigger}"].sum().sum()
    output["reco_cutflow"][dataset_name]["probe"] += selections[f"probe_{self._nominal_trigger}"].sum().sum()
    ...
    truth_selections["matched_tag"][truth_selections["matched"]] = selections[f"tag_{self._nominal_trigger}"][truth_bukmumu.reco_idx[truth_selections["matched"]]]
    truth_selections["matched_probe"] = truth_bukmumu.reco_idx.zeros_like().astype(bool)
    truth_selections["matched_probe"][truth_selections["matched"]] = selections[f"probe_{self._nominal_trigger}"][truth_bukmumu.reco_idx[truth_selections["matched"]]]
    ...
    for selection_name in self._Bcand_selections:
      output[f"Bcands_{selection_name}"][dataset_name].extend({
        ...
        "mass": reco_bukmumu.fit_best_mass[selections[selection_name]].flatten(),
